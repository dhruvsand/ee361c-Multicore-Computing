...
public class LockBasedRBTreeGUI extends JFrame {

	private JPanel contentPane;
	public FineGrainedRBTree tree;
	public DrawLockedTree drawer;
	
	/**
	 * Create the frame.
	 */
	public LockBasedRBTreeGUI(FineGrainedRBTree tree) {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 1000, 1000);
		contentPane = new JPanel();
		contentPane.setBorder(new EmptyBorder(5, 5, 5, 5));
		contentPane.setLayout(new BorderLayout(0, 0));
		contentPane.setBorder(BorderFactory.createTitledBorder("Lock-Based Tree"));
		drawer = new DrawLockedTree(tree);
...
class DrawLockedTree extends JPanel{
	
	public FineGrainedRBTree tree;
	
	public DrawLockedTree(FineGrainedRBTree tree){
		this.tree = tree;
	}
	...
	protected void paintComponent(Graphics g) {
		...
		DrawLockedTree(g, 0, getWidth(), 0, getHeight() / tree.getheight(tree.root), tree.root);
	}
	

	
    public void DrawLockedTree(Graphics g, int StartWidth, int EndWidth, int StartHeight, int Level, FineGrainedRBNode node) {
        String data = "";
        ...
        if (node.getLeft() != null){
        		...
        		DrawLockedTree(g, StartWidth, (StartWidth + EndWidth) / 2, StartHeight + Level, Level, node.getLeft());
        }
        ...
        if (node.getRight() != null){
        		...
        		DrawLockedTree(g, (StartWidth + EndWidth) / 2, EndWidth, StartHeight + Level, Level, node.getRight());
        }
    }
